...
La activación utiliza `MELI_CLIENT_ID` y `MELI_CLIENT_SECRET` guardados como secretos del repositorio. En cada ejecución el conector solicita un token de aplicación mediante Client Credentials y lo mantiene solo en memoria. Si faltan credenciales o la API las rechaza, el flujo conserva el último archivo válido, no falla los demás conectores y el sitio informa que Mercado Libre sigue pendiente. Los secretos y tokens nunca deben añadirse al repositorio ni enviarse por chat.
