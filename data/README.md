...
La aplicación utiliza `MELI_CLIENT_ID` y `MELI_CLIENT_SECRET` guardados como secretos del repositorio, pero Mercado Libre no admite `client_credentials` para este flujo. La activación requiere completar Authorization Code OAuth y guardar el token como secreto `MELI_ACCESS_TOKEN`. Mientras no exista, el proceso conserva el estado pendiente, no falla los demás conectores y el sitio informa que no hay datos de Mercado Libre. Los secretos y tokens nunca deben añadirse al repositorio ni enviarse por chat.
